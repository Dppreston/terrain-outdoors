...
router.put("/users", async (req: Request, res: Response) => {
  ...
  const check = user[0].cart.find((el: Cart) => el.productId === productId);
  ...
  if (handleWishlist) {
    ...
    const check = user[0].wishlist.find(
...
router.get("/products", async (req: Request, res: Response) => {
  ...
  if (pEnv) {
    const viewAllArray = (sub as string).split(" ");
    ...
    if (viewAllCheck === false && saleCheck === false) {
      ...
      if (productData) {
        if (sortSelection === "undefined") {
          ...
          if (ss === 2) {
            return res.send(
              JSON.stringify([
                limitedProductData.sort(
                  (a: PD, b: PD) => Number(b.price) - Number(a.price)
                ),
                filterData,
              ])
            );
          ...
          if (ss === 3) {
            return res.send(
              JSON.stringify([
                limitedProductData.sort(
                  (a: PD, b: PD) => Number(a.price) - Number(b.price)
                ),
                filterData,
              ])
            );
          ...
          if (ss === 4) {
            ...
            limitedProductData.forEach((product: PD) => {
              let sum = 0;
      ...
      const saleItems = allProducts.filter(
        (el: { sale: Array<any> }) => el.sale[0] === true
      );
  ...
  if (filterProducts) {
    ...
    let end =
      Number(selectedPage) !== 1 && selectedPage != "undefined"
    ...
    const brandsArr = parsedFilters
      .filter((el: { title: string }) => el.title === "brand")
      .flatMap((el: { option: string }) => el.option);
    const colorArr = parsedFilters
      .filter((el: { title: string }) => el.title === "color")
      .flatMap((el: { option: string }) => el.option);
    ...
    if (viewAllCheck === false && saleCheck === false) {
      ...
      if (brandsArr.length > 0) {
        for (let i = 0; i < brandsArr.length; i++) {
          ...
          if (brandsArr.length > 0 && colorArr.length > 0) {
            ...
            return res.send(
              JSON.stringify(colorsArraySelected.slice(start, end))
            );
          }
        }
      ...
      const fetchSale = fetchAll.filter(
        (el: { sale: Array<any> }) => el.sale[0] === true
      );
      if (brandsArr.length > 0) {
        for (let i = 0; i < brandsArr.length; i++) {
          const filterBrand = fetchSale.filter(
            (el: { brand: string }) => el.brand === brandsArr[i]
          );
          ...
          if (brandsArr.length > 0 && colorArr.length > 0) {
            for (let y = 0; y < colorArr.length; y++) {
              const colorFilter = filteredProductsArr.filter(
                (el) => el.color === colorArr[y]
              );
        ...
        /// COLOR ONLY
        for (let x = 0; x < colorArr.length; x++) {
          const colorFiltered = fetchSale.filter(
            (el: { color: string }) => el.color === colorArr[x]
          );
  ...
  if (productSort) {
    ...
    if (ss === 2) {
      return res.send(
        JSON.stringify([
          data.sort((a: PD, b: PD) => Number(b.price) - Number(a.price)),
        ])
      );
    }
    ...
    if (ss === 3) {
      return res.send(
        JSON.stringify([
          data.sort((a: PD, b: PD) => Number(a.price) - Number(b.price)),
        ])
      );
    }
    ...
    if (ss === 4) {
      ...
      return res.send(
        JSON.stringify([topRatingArray.sort((a, b) => b.rating - a.rating)])
      );
    }
  }
  ...
  if (initCart) {
    ...
    const ids = (productId as string).split(",");
    const quan = (quantity as string).split(",");
    if (ids[0] != "") {
  ...
  if (initReviews) {
    ...
    if (product && (filters == "undefined" || filters?.length === 0)) {
      ...
      return res.send(
        JSON.stringify([limitedReviews, reviewsLength, reviewBarData])
      );
    } else {
      //filter reviews
      const filteredReviewData = new Array();
      const limitedFilteredData = new Array();
      const filtersArr = (filters as string).split(",");

      for (let i = 0; i < filtersArr.length; i++) {
        let filtered = product[0].reviews.filter(
          (el: { rating: number }) => el.rating === Number(filtersArr[i])
        );
  ...
  if (loadMoreReviews) {
    ...
    if (filters === "undefined" || filters?.length === 0) {
      const updatedReviewData = product[0].reviews.splice(
        0,
        Number(existingLength) + 10
      );
      ...
      for (let i = 0; i < filtersArr.length; i++) {
        ...
        limitedFilteredData.push(
          ...filtered.splice(0, Number(existingLength) + 10)
        );
      }
  ...
  if (promo) {
    ...
    if (promoType === "2") {
      ...
      const ratingDataArr: { _id: string; total: number }[] = productData
        .filter((all: PD) => all.reviews.length != 0)
        .flatMap((data: { _id: string; reviews: { rating: number }[] }) => [
          {
            _id: data._id,
            total:
              data.reviews
      ...
      if (ratingDataArr.length > 0) {
        ...
        return res.send(
          JSON.stringify(
            ratingArrFinal.sort((a, b) => b[1] - a[1]).splice(0, 10)
          )
        );
      }
    }
    ...
    if (promoType === "3") {
      let filteredSaleItems = productData.filter((data: PD) =>
        data.sale.includes(true)
      );
  ...
  if (topSellers) {
    ...
    const topSellerIdsParsed = JSON.parse(topSellerIds as string);

    for (let i = 0; i < topSellerIdsParsed.length; i++) {
      const productData = await products.find(
        { _id: topSellerIdsParsed[i] },
        {
          title: 1,
          price: 1,
          previewImg: 1,
  ...
  if (search) {
    ...
    const searchSplit = (searchContent as string).toLowerCase().split("");
    const wordSplit = (searchContent as string).toLowerCase().split(" ");

    /// filter for specific word

    const filterProductsWord = allProductData.filter((word: PDSearch) =>
      wordSplit.every(
        (full: string) =>
          word.title.toLowerCase() === full ||
    ...
    if (filterProductsWord.length > 0) {
      ...
      const filterProductsChar = allProductData.filter((word: PDSearch) =>
        searchSplit.every(
          (char) =>
...
router.put("/products", async (req: Request, res: Response) => {
  ...
  if (addReview) {
    const parsedReviewData = JSON.parse(reviewData as string);
    if (parsedReviewData) {
      const action = await products.updateOne(
        { _id: productId },
        {
          $push: {
            "reviews": {
...
router.post("/orders", async (req: Request, res: Response) => {
  ...
  if (submitOrder) {
    const infoParsed: OrderInfo = JSON.parse(info as string);
    const shippingParsed: ShippingInfo = JSON.parse(shipping as string);
    const paymentParsed: PaymentInfo = JSON.parse(payment as string);
    const cartParsed: CartFinal = JSON.parse(cart as string);
...
router.get("/orders", async (req: Request, res: Response) => {
  ...
  if (promo) {
    ...
    if (promoType === "1") {
      ...
      allOrderData.forEach((order: { items: Array<any> }) =>
        order.items.forEach((item: { _id: string }) =>
          orderItemArray.push(item._id)
        )
      );
      ...
      if (orderItemArray.length > 0) {
        ...
        Object.entries(count).forEach((el) =>
          freqArray.push({ _id: el[0], count: el[1] })
        );
